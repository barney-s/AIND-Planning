...
## Search methods
These are the different search methods used to find a solution to the problems. The optimal solution should be recorded and searches should be analysed against various metrics.
...
|---|---|---|---|
|Breadth First (graph)|being optimal search a solution is always found. This search used more space than other searches|optimal search|more space used|
...
|Uniform Cost|Since node costs were all equal(to 0), the performance was similar to BFS graph. Optimal solution was found. But used slightly more memory and time compared to BFS graph as the goal checks are applied only when nodes are pop'd from the fontier.|Optimal search|More space used|
...
*A\* with pg\_levelsum* took longer time but expanded less nodes to find optimal solution for the first 2 problems. For the 3rd problem, it timed out (600s).


### Best Heuristic for this problem
